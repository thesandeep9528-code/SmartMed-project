...
import { Search, MapPin, AlertTriangle, Info, Clock, ExternalLink, ActivitySquare, CheckCircle2, Locate, Hospital, Navigation, Loader2 } from 'lucide-react';
...
export default function Book() {
  ...
  // Group 1: AI Auto-Routing based on symptoms
  useEffect(() => {
     if (symptoms.length > 5 && !selectedDoctor) {
         const bestDoc = matchDoctorBySymptom(symptoms);
         if (bestDoc) setSelectedDoctor(bestDoc.id);
  ...
  useEffect(() => {
     if (selectedDoctor) {
         ...
         // Group 1: Micro-Slot Booking Generator
         setMicroSlot(generateMicroSlot(wait));
     }
  }, [selectedDoctor, priority]);
        ...
        <div className="bg-white p-8 rounded-2xl shadow-xl border border-gray-100 relative overflow-hidden">
          ...
          <div className="text-center mb-8">
            <h2 className="text-3xl font-extrabold text-gray-900">
              AI Smart Booking & Triage
            </h2>
            <p className="mt-2 text-sm text-gray-500">
               Enter your symptoms to let our AI Load Balancer automatically route you to the fastest available specialist.
            </p>
          </div>

          <form className="space-y-6" onSubmit={handleBooking}>
            
            {/* Group 1: Pre-Consultation AI Form */}
            <div>
              <label className="block text-sm font-bold text-gray-900 mb-2 flex items-center gap-2">
                 <ActivitySquare className="w-4 h-4 text-primary-500" /> Patient Symptoms (AI Analyzed)
              </label>
              <textarea
                ...
                onChange={(e) => setSymptoms(e.target.value)}
                rows={2}
                className="appearance-none rounded-xl relative block w-full px-3 py-3 border border-gray-300 placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm bg-gray-50 transition-colors"
                placeholder="E.g., I have been experiencing severe chest pain and dizziness since morning..."
              />
              {symptoms.length > 5 && (
                  <p className="text-xs text-success-600 mt-2 flex items-center gap-1 font-medium animate-fade-in">
                     <CheckCircle2 className="w-3 h-3" /> AI Engine successfully matched specialty.
                  </p>
              )}
            </div>

            {/* Mood/Priority Selection */}
            <div className="bg-surface-light p-4 rounded-xl border border-gray-100">
               <label className="block text-sm font-bold text-gray-900 mb-3 flex items-center gap-2">
                  Patient Condition <span className="text-xs font-normal text-gray-500 bg-white px-2 py-0.5 rounded-full shadow-sm border border-gray-200">Mood-Based Priority</span>
               </label>
            ...
            {/* Smart Modules appear only when doctor is selected */}
            {selectedDoctor && (
               <div className="space-y-4 animate-fade-in">
                  
                  {/* Micro-Slot & Dynamic Wait Time Predictor (Game Changer) */}
                  <div className="flex bg-blue-50 border border-blue-200 rounded-xl p-4 gap-4 items-center">
                     <div className="bg-blue-600 p-3 rounded-xl text-white shadow-md">
                        <Clock className="w-6 h-6" />
                     </div>
                  ...
                  {/* Smart Right Time Predictor (Game Changer) */}
                  <div className="bg-success-500/5 rounded-xl p-4 border border-success-500/20 text-success-700 flex items-start gap-3">
                     <Info className="w-5 h-5 flex-shrink-0 mt-0.5 text-success-600" />
                     <p className="text-sm font-medium leading-relaxed">{timePrediction}</p>
                  </div>

                  {/* Smart Hospital Switching (Game Changer) */}
                  {smartSuggestion && (
                     <div className="bg-orange-50 rounded-xl p-4 border border-orange-200">
                        <div className="flex items-start gap-3 text-orange-800">
                           <AlertTriangle className="w-5 h-5 flex-shrink-0 mt-0.5 text-orange-600" />
                           <div className="flex-1">
                              ...
                              <button 
                                 type="button"
                                 onClick={() => setSelectedDoctor(smartSuggestion.doctorId)}
                                 className="inline-flex items-center w-full justify-center gap-2 px-3 py-2 bg-orange-500 hover:bg-orange-600 text-white text-sm font-bold rounded-lg transition-colors shadow-sm cursor-pointer"
                              >
                                 Auto-Switch & Save {smartSuggestion.savedTime} mins <ExternalLink className="w-4 h-4" />
                              </button>
                           </div>
            ...
            <div>
              <button
                type="submit"
                className="group relative w-full flex justify-center py-4 px-4 border border-transparent text-lg font-bold rounded-xl text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 shadow-md shadow-primary-500/30 transition-all mt-8"
              >
                Confirm Priority AI Booking
              </button>
            </div>
